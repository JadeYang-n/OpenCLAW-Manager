...
export default function DashboardPage() {
    ...
    try {
      ...
      try {
        ...
        const statsData: InstanceStats = {
          ...
          degraded: instances.filter((i) => (i as { status?: string })?.status === 'degraded').length,
        }
        setStats(statsData)
        ...
        offlineInstances.forEach((inst, index) => {
          generatedAlerts.push({
            id: `offline-${index}`,
            timestamp: '实时',
            message: `${(inst as { name?: string })?.name || 'Unknown'} ${t('dashboard.instanceOffline')}`,
            severity: 'critical',
          })
        })
        ...
        setAlerts([{
          id: 'error',
          timestamp: '实时',
          message: t('dashboard.cannotLoadInstances'),
          severity: 'info',
        }])
      }
    } catch (err) {
      ...
      setAlerts ([{
        id: 'error',
        timestamp: '实时',
        message: t('dashboard.cannotLoadData'),
        severity: 'info',
      }])
    } finally {
      setLoading(false)
  ...
  const loadUsageData = useCallback(async () => {
    try {
      ...
      const [usageRes, quotaRes] = await Promise.all([
        ...
        usageAPI.getQuotaConfig(),
      ]) as any
      if (usageRes.success && usageRes.data) setUsageStats(usageRes.data)
      if (quotaRes.success && quotaRes.data) setQuotaConfig(quotaRes.data)
    } catch (error) {
  ...
  const getUserRoleText = () => {
    ...
    if (user?.role === 'dept_admin') return { text: '部门管理员', icon: <Users className="w-4 h-4" /> }
    if (user?.role === 'employee') return { text: '员工', icon: <Activity className="w-4 h-4" /> }
    if (user?.role === 'auditor') return { text: '审计员', icon: <Activity className="w-4 h-4" /> }
  ...
  return (
    <div className="min-h-screen bg-background p-6 animate-fade-in">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-2">
              {t('dashboard.welcome')} <span className="text-primary">OpenCLAW Manager</span>
            </h1>
        ...
        <Card variant="premium">
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-muted-foreground font-medium">{t('dashboard.totalInstances')}</p>
        ...
        <Card variant="premium">
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-muted-foreground font-medium">{t('dashboard.onlineInstances')}</p>
        ...
        <Card variant="premium">
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-muted-foreground font-medium">{t('dashboard.offlineInstances')}</p>
        ...
        <Card variant="premium">
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-muted-foreground font-medium">{t('dashboard.degradedInstances')}</p>
            ...
            <div className="space-y-3">
              ...
              {alerts.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground text-lg">{t('dashboard.systemNormal')}</p>
                  <p className="text-sm text-muted-foreground mt-1">{t('dashboard.noAlerts')}</p>
                </div>
              )}
            </div>
          </CardContent>
        ...
        <Card variant="premium">
          ...
          <CardContent>
            <div className="grid grid-cols-1 gap-3">
              {(user?.role === 'admin' || user?.role === 'operator' || user?.role === 'dept_admin') && (
                <Button
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="font-semibold text-lg">{t('dashboard.manageInstances')}</div>
                    <div className="text-sm text-primary-foreground/80 mt-1">
                      {t('dashboard.manageInstancesDesc')}
                    </div>
                  </div>
                </Button>
              )}
              ...
              {(user?.role === 'admin' || user?.role === 'operator') && (
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="font-semibold text-lg">{t('dashboard.deployNew')}</div>
                    <div className="text-sm text-success-foreground/80 mt-1">
                      {t('dashboard.deployNewDesc')}
                    </div>
                  </div>
                </Button>
              )}

              {(user?.role === 'admin' || user?.role === 'operator' || user?.role === 'dept_admin' || user?.role === 'auditor') && (
                <Button
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="font-semibold text-lg">{t('dashboard.auditLogs')}</div>
              ...
              {user?.role === 'admin' && (
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="font-semibold text-lg">{t('dashboard.userManagement')}</div>
                    <div className="text-sm text-muted-foreground mt-1">
      ...
      {/* 部门维度用量 */}
      {usageStats && usageStats.by_department && usageStats.by_department.length > 0 && (user?.role === 'admin' || user?.role === 'operator') && (
        <Card variant="premium" className="mb-6">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-primary" />
              {t('dashboard.deptUsage')}
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <div className="space-y-5">
                <div>
                  <div className="flex justify-between mb-2">
                    <span className="text-sm text-muted-foreground">{t('dashboard.budgetRate')}</span>
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center p-6 bg-gradient-to-br from-muted/50 to-background rounded-xl">
              ...
              <p className="text-sm text-muted-foreground">{t('dashboard.criticalAlerts')}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="mt-6 p-4 bg-primary/5 border border-primary/10 rounded-lg flex items-start gap-3">
        ...
        <div>
          <h4 className="font-medium text-foreground mb-1">{t('dashboard.workbenchNote')}</h4>
          <p className="text-sm text-muted-foreground">
            {t('dashboard.workbenchDesc')}
          </p>
        </div>
      </div>
    </div>
