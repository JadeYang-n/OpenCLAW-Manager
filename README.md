...
[![Version](https://img.shields.io/badge/version-v1.0.0--Beta-green.svg)](https://github.com/JadeYang-n/OpenCLAW-Manager)
...
### 💰 Token 成本分析
- ✅ 多路径采集：Gateway 补丁 / Session Watcher / Prometheus 轮询 / 手动上报
- ✅ 按部门/业务线分摊
- ✅ 每日趋势图表
...
- ✅ Skill 审核（自动 + 人工 + 审计轨迹）
- ✅ Skill 商店（按部门/可见性过滤）
- ✅ Skill 远程调用（WebSocket RPC + HTTP + CLI 三级降级）
- ✅ 参数 Schema（调用时自动生成表单）
...
### ✅ v1.0 - 基础权限（Beta 已发布）
- [x] 用户登录/登出（JWT 认证）
- [x] 5 角色权限系统（后端 RBAC 强制实施）
- [x] 实例管理 CRUD
- [x] 操作审计日志
- [x] Token 分析页面（多路径采集端到端打通）
- [x] 部门管理 API + 实例/用户部门绑定

### ✅ v2.0 - 企业级特性（Beta 已发布）
- [x] Gateway 在线状态检测 + 自动配置
- [x] Token 计费 + 成本分析
- [x] Skill 商店（上架/审核/远程调用）
- [x] LAN 自动发现（mDNS）+ 远程配对
- [x] Skill 参数 Schema（自动生成调用表单）
- [x] 集成测试（44 个测试全部通过）
- [x] API 文档（Swagger UI at `/api-docs/`）
- [x] Tauri 命令补全（14 个 stub 全部实现）
- [x] 跨机器 Token 采集脚本（ocm-agent/collector/）
